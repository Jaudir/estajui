...
//    if (isset($_POST["matricula"]) && isset($_POST["semestre"]) && isset($_POST["serie"]) && isset($_POST["modulo"]) && isset($_POST["periodo"]) && isset($_POST["integralizacao"]) && isset($_POST["dependencias"]) && isset($_POST["aptidao"]))
    if (!empty($_POST["matricula"]) && !empty($_POST["semestre"]) && isset($_POST["aptidao"]) && !empty($_POST["justificativa"])) {
        ...
        if (is_array($estagio)) {
            if (count($estagio) > 0) {
                ...
                $matricula->setmatricula($_POST["matricula"]);
                $matricula->setsemestre_inicio($valorestmp[0]);
                $matricula->setano_inicio($valorestmp[1]);
                ...
                $matricularesult = $matriculaModel->update($matricula);
                if (!$matricularesult) {
                    $estagio->setmatricula($matricula);
                    if (!$estagioModel->update($estagio) && $modificacaostatusModel->create($modicacao)) {
        ...
        if (empty($_POST["matricula"])) {
            ...
            $session->pushError("Avalie se o o aluno está apto a realizar o estágio!", "error-validacao");
        } else {
